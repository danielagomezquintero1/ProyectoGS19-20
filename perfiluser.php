...
//seguidores
	//si ese registro ya existe, para no añadir mas registros a la bd
	$yaExiste = $mysqli->query("SELECT * FROM seguidores WHERE seguidor='".$_SESSION['id']."'");
	 //btener cauntos registros nos da
	 $yaEnviaste = $yaExiste->num_rows; 

   if(isset($_GET['seguir'])){ //cuando le damos a seguir

    
	 //verificamos si ya envio la notificaion
	 if($yaEnviaste>0){
	 	echo "Le sigues";
	 }	else
	 	{
		   	if($rowA['private_profile'] == 1)//si el perfil es privado
		   	{
		   		$aprobado=1; //le peudes seguir sin problemas
		   	} else{
		   		$aprobado=1; //Cuando el perfil ya es publico, la solicitud ya ha sido aprobado, le peudes seguir sin problemas
		   	}

		   	$sqlG = $mysqli->query("INSERT INTO seguidores (seguidor, seguido, aprobada, fecha) VALUES ('".$_SESSION['id']."','".$rowA['id']."', '$aprobado',now())");

		   	if($sqlG){
		   		header("Location:perfiluser.php?username=".$_GET['username']."");
	   		 }
	   	}	
   }//nosotros le seguimos al id de ese usuario
...
<style type="text/css">
  ...
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active" aria-current="page">Tú perfil</li>
  </ol>
</nav>    
 ...
 <?php
        ...
        <div class="col-12"style="background-color: white;">
          <div class="row">
            <div class="col-2 mt-4">
              <?php echo"<img src=".$rowA["avatar"]." style='width:110%'>"; ?>
            </div>
              ...
              <div class="row">
                ...
                <?php } else{ //SI el perfil es de otro usuario, saldra, seguir  ?>
                  <div class="col-5">
                  	<!--boton seguir-->
                  	<?php if($yaEnviaste > 0){  ?>
                    	<div class="mt-3"><button class="btn btn-info btn-sm">Le sigues</button></div>
                    <?php }  else{ ?>
                    	<a href="?seguir=seguir&username=<?php echo $_GET['username']; ?>"><div class="mt-3"><button class="btn btn-info btn-sm">Seguir</button></div></a>
                	<?php } ?>
                  </div>
                <?php }  ?>
                <div class="col-12">
                  <div class="row">
                    <div class="col-3">
                      <small style="color:black;"><span><?php echo $result3 ?> </span>Seguidores</small>
                    </div>
                    <div class="col-3">
                     <small style="color:black;"><span><?php echo $result4 ?>  </span>Seguidos</small>
                   </div>
                 </div>
               </div>
    ...
    <div class="row mt-5">
      ...
      <div class="col-12 p-3 m-2" style="background-color: white;">
        <div class="col-3" style="text-align: left;"><a href="cancionesfavoritas.php?id=<?php echo $_SESSION['id']; ?>" class="btn btn-danger btn-sm">Ver tus canciones favoritas</a></div>
        <h4>Numero de canciones: <?php echo $result2; ?><br></h4>

        <!--mostramos las publicaciones-->

        <!--si el perfil es privado y la session no es mia...(no soy ese usuario) no puedo ver el perfil del usuario-->
        <?php 
